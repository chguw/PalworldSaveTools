...
def _hash_len17to32(s, length):
    ...
    return _hash_len16_mul(
        u64(_rotate(u64(a + b), 43) + _rotate(c, 30) + d),
        u64(a + _rotate(u64(b + k2), 18) + c),
        mul
    )
...
def _weak_hash_len32_with_seeds(s, a, b):
    return _weak_hash_len32_with_seeds_6(
        _fetch64(s[0:8]), _fetch64(s[8:16]),
        _fetch64(s[16:24]), _fetch64(s[24:32]),
        a, b
    )

def _byteswap64(x):
    return struct.unpack('<Q', struct.pack('>Q', x & 0xffffffffffffffff))[0]
...
def cityhash64(data):
    ...
    z = _hash_len16(
        _fetch64(s[length-48:length-40]) + length,
        _fetch64(s[length-24:length-16])
    )
    ...
    return _hash_len16(
        u64(_hash_len16(v[0], w[0]) + u64(_shift_mix(y) * k1) + z),
        u64(_hash_len16(v[1], w[1]) + x)
    )
