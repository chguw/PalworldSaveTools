...
def run_convert(sav_path):
    ...
    proc = subprocess.run(
        [sys.executable, '-m', 'palworld_save_tools.commands.convert',
         str(sav_path), '--to-json', '-o', output, '--force'],
...
def run_roundtrip(sav_path):
    ...
    proc_json = subprocess.run(
        [sys.executable, '-m', 'palworld_save_tools.commands.convert',
         str(sav_path), '--to-json', '-o', temp_json, '--force'],
        capture_output=True, text=True,
        cwd=get_pst_root(),
        env=env,
        timeout=120,
    ...
    proc_sav = subprocess.run(
        [sys.executable, '-m', 'palworld_save_tools.commands.convert',
         temp_json, '--from-json', '-o', temp_sav, '--force'],
        capture_output=True, text=True,
        cwd=get_pst_root(),
        env=env,
        timeout=120,
...
def parse_error(stderr, stdout):
    output = stderr + '\n' + stdout
    match = re.search(r'Exception: (.+)', output)
    if not match:
        return None
    ...
    result = {
        ...
        'stack_frames': frames,
        'file_line': frames[-1] if frames else None,
    }
    ...
    if 'EOF not reached for module type' in error_msg:
        ...
        type_match = re.search(r"Unknown property value type: (\w+)", error_msg)
        if type_match:
...
FIX_PATTERNS = {
    'map_concrete_model_module_decode_eof': {
        'old': (
            '    if not reader.eof():\n'
    ...
    'map_concrete_model_module_encode_default': {
        'pattern': 'case \'EPalMapObjectConcreteModelModuleType::RequireElementalAction\':',
        'insert_before': '    encoded_bytes = writer.bytes()',
        'insert': (
            "        case _:\n"
    ...
    'map_concrete_model_eof': {
        'pattern': "if not reader.eof():\n        raise Exception(f\"Warning: EOF not reached for",
        'replace_lines': True,
    },
    'encode_append_unknown_bytes': {
        'pattern': '    encoded_bytes = writer.bytes()',
        'insert_before': '    encoded_bytes = writer.bytes()',
        'insert': (
...
def detect_variable_name(content, raise_line_idx):
    ...
    if raise_line_idx + 1 < len(lines):
        rm = re.match(r'^\s+return\s+(\w+)', lines[raise_line_idx + 1])
        if rm:
            return rm.group(1)
    ...
    for i in range(raise_line_idx - 1, start - 1, -1):
        ...
        if m:
            ...
            if candidate not in ('guild',):
                return candidate
        m = re.match(r"^(\w+):\s*dict", stripped)
        if m:
            return m.group(1)
        m = re.match(r"^(\w+)\s*\|\|?=\s*\{", stripped)
        if m:
...
def handle_eof_replacement(file_path, changelog, dry_run):
    ...
    indent = re.match(r'^(\s*)', if_line_text).group(1)
    old = if_line_text + '\n' + lines[eof_line_idx]
    new_lines_str = (
        f"{indent}if not reader.eof():\n"
        f"{indent}    {var_name}['unknown_bytes'] = [int(b) for b in reader.read_to_end()]"
    )
...
PROP_VALUE_WRITER_CALLS = {
    'Int64Property': 'self.i64(value)',
    'UInt64Property': 'self.u64(value)',
    'UInt16Property': 'self.u16(value)',
    'FixedPoint64Property': 'self.i32(value)',
    'FloatProperty': 'self.float(value)',
    'ByteProperty': 'self.byte(value)',
    'DoubleProperty': 'self.double(value)',
    'Int16Property': 'self.i16(value)',
}
...
def handle_prop_value_fix(file_path, missing_type, changelog, dry_run):
    ...
    reader_replacement = (
        "        elif type_name == 'StrProperty':\n"
        "            return self.fstring()\n"
        f"        elif type_name == '{missing_type}':\n"
        f"            return {reader_call}\n"
        "        else:\n"
        "            raise Exception(f'Unknown property value type: {type_name} ({path})')"
    )
...
def harden_all_modules(dry_run):
    changelog = ChangeLog()
    rawdata_dir = get_src_dir() / 'palworld_save_tools' / 'rawdata'
    py_files = sorted(rawdata_dir.glob('*.py'))

    match_based_encode_files = {
    ...
    for file_path in py_files:
        ...
        encode_fixed = handle_encode_fix(
            str(file_path),
            fname in match_based_encode_files,
            changelog,
            dry_run,
        )
...
def main():
    parser = argparse.ArgumentParser(
        description='Auto-update PalWorld Save Tools for new save formats.\n'
                    'Detects structural changes in new .sav files and patches\n'
                    'the palworld_save_tools source code to handle them.',
    ...
    parser.add_argument('--harden-only', action='store_true',
                        help='Only harden rawdata modules against crashes (no iterative fix loop)')
    ...
    for iteration in range(1, MAX_ITERATIONS + 1):
        ...
        if error['type'] == 'unknown_property_value_type':
            ...
            archive_path = get_src_dir() / 'palworld_save_tools' / 'archive.py'
            if missing_type and archive_path.exists():
                fixed = handle_prop_value_fix(
                    str(archive_path), missing_type, fix_changelog, args.dry_run
                )
