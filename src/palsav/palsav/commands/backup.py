...
def main():
    ...
    export_player_parser = subparsers.add_parser(
        "export-player", help="Export player backup"
    )
    ...
    export_player_parser.add_argument(
        "-o", "--output", required=True, help="Output path (.player.pstz)"
    )
    export_player_parser.add_argument(
        "--no-compress",
        action="store_true",
        help="Export as uncompressed JSON instead of OOZ compressed",
    )
    export_player_parser.add_argument(
        "-f", "--force", action="store_true", help="Force overwrite output file"
    )
    ...
    export_base_parser.add_argument(
        "-o", "--output", required=True, help="Output path (.base.pstz or .json)"
    )
    export_base_parser.add_argument(
        "--no-compress",
        action="store_true",
    ...
    import_player_parser = subparsers.add_parser(
        "import-player", help="Import player backup"
    )
    import_player_parser.add_argument(
        "backup", help="Path to player backup file (.player.pstz)"
    )
    ...
    import_player_parser.add_argument(
        "--no-compress", action="store_true", help="Backup is uncompressed JSON"
    )

    import_base_parser = subparsers.add_parser("import-base", help="Import base backup")
    import_base_parser.add_argument(
        "backup", help="Path to base backup file (.base.pstz or .json)"
    )
    ...
    import_base_parser.add_argument(
        "--offset", nargs=3, type=int, default=[8000, 0, 0], help="Base offset (x y z)"
    )
    ...
    if args.command == "export-player":
        ...
        if os.path.exists(args.output) and not args.force:
            logger.error(
                f"Output file {args.output} already exists. Use -f to overwrite."
            )
            exit(1)
        try:
            export_player_backup(
                args.level_sav, args.player_uid, args.output, compressed=compressed
            )
        ...
        try:
            export_base_backup(
                args.level_sav, args.base_id, args.output, compressed=compressed
            )
        ...
        try:
            import_player_backup(
                args.backup, args.level_sav, args.players_folder, compressed=compressed
            )
