...
from palworld_aio.base_manager import (
    export_base_json,
    import_base_json,
...
def compress_to_pst3(data):
    ...
    brotli_bytes = brotli.compress(
        cbor_bytes, mode=brotli.MODE_TEXT, quality=11, lgwin=24
    )
...
def export_base_backup(level_sav_path, base_id, output_path, compressed=True):
    ...
    if compressed:
        output_path = (
            output_path.replace(".pstz", ".pstbase")
            if output_path.endswith(".pstz")
            else output_path
        )
        ...
        json_path = (
            output_path.replace(".pstbase", ".json")
            if output_path.endswith(".pstbase")
...
def import_base_backup(
    pstz_path,
    target_level_sav_path,
    target_guild_id,
    compressed=True,
    offset=(8000, 0, 0),
):
...
def export_player_backup(level_sav_path, player_uid, output_path):
    ...
    if not player_cspm_entry:
        ...
        for entry in cspm[:50]:
            try:
                ...
                pass
        raise Exception(
            f"Could not find player {player_uid} in CharacterSaveParameterMap"
        )
    ...
    if not os.path.exists(player_sav_path):
        alt_path = os.path.join(
            os.path.dirname(level_sav_path), "../Players", f"{player_uid_clean}.sav"
        )
    ...
    backup_data = {
        "_version": VERSION,
        "_player_uid": str(player_uid),
        "cspm_entry": player_cspm_entry,
        "player_sav_properties": player_gvas.properties,
        "player_sav_header": player_gvas.header.dump(),
        "player_sav_trailer": base64.b64encode(player_gvas.trailer).decode("utf-8"),
    }
...
def import_player_backup(pst7_path, target_level_sav_path):
    ...
    if not player_sav_header:
        raise Exception(
            "Backup file is missing player_sav_header. The backup may be corrupted or from an incompatible version."
        )
    ...
    target_players_folder = os.path.join(
        os.path.dirname(target_level_sav_path), "Players"
    )
    ...
    player_gvas = GvasFile.load(
        {
            "header": player_sav_header,
            "properties": player_sav_props,
            "trailer": player_sav_trailer
            if player_sav_trailer
            else base64.b64encode(b"\x00\x00\x00\x00").decode("utf-8"),
        }
    )
