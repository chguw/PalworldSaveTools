#2.0.8
- **Fixed `LocalData.sav` parsing** — `PalLocalWorldSaveGame` class saves now parse correctly. Root cause: `.SaveData.Local_MaxFriendshipPalIds.Key` type hint was `"Guid"` (16B), but actual data is a full StructProperty containing `PlayerUId`. Caused parser to under-consume each map entry, cascading into over-consumption and skipping all subsequent properties. Fixed by changing hint to `"StructProperty"` so `properties_until_end` correctly reads the key structure.
...
- **Restore Map now removes cloud overlay** — `clear_fog_in_local_data` sets `Local_ShowSkyIslandCloudOnWorldMapUI` to `false`, so the revealed map isn't blocked by the sky island cloud layer.
- **Updated `pst-binary-schemas` skill** — documents v2 guild tail (`_read_guild_tail_v2`), role semantics (1=admin, 2=submaster, 3=member), and all fixed files. Added type hint debugging methodology section.
- **Fixed cwd-dependent relative paths (system32 crash)** — all `base_path = '.'` and `'./saves'` hardcodes replaced with `get_data_base()` from `resource_resolver`. 15 sites across 6 files: `import_libs.py`, `restore_map.py`, `dialogs.py`, `main.py`, `func_manager.py`, `save_manager.py`, `game_pass_save_fix.py`. Fixes `PermissionError: [WinError 5] Access is denied: 'C:\WINDOWS\system32\Backups'` when app runs elevated
- **`get_data_base()` split from `get_base_dir()`** — `get_base_dir()` continues resolving to repo root / Nuitka temp dir (for resources); new `get_data_base()` returns `%APPDATA%/PalworldSaveTools/` (frozen) or repo root (dev). Prevents Backups/Logs from being lost in Nuitka onefile builds and avoids system32 permission errors. Detection uses `sys.frozen` or non-python executable name for Nuitka onefile compatibility
- **Config persistence for frozen builds** — added `get_user_config_dir()` resolving to `%APPDATA%\PalworldSaveTools\configs\` (Win), `~/.config/PalworldSaveTools/configs/` (Linux), or `~/Library/Application Support/PalworldSaveTools/configs/` (macOS) when frozen. Dev mode unchanged. Affected files: `config.json`, `user.cfg`, `passive_loadouts.json`, `inventory_loadouts.json`, `equipment_loadouts.json`, `base_inventory_loadouts.json`. Defaults auto-migrated from bundled configs on first run via `_migrate_configs()`, now called from `palworld_aio/main.py` (was only in `bootup.py:main()` which never runs in frozen builds — migration silently skipped). Fixes Nuitka onefile losing settings (console detach, language, loadouts) on restart
